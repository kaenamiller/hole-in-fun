...
const FACILITIES: Array[String] = ["clubhouse", "driving_range", "restroom", "snack_kiosk", "cart_barn", "maintenance_shed"]
const SCENERY: Array[String] = ["oak_tree", "pine_tree", "woodland_log", "woodland_boulder", "pergola", "fountain", "flower_bed", "topiary", "gazebo", "palm_tree", "bench", "decorative_pond"]
...
func _init() -> void:
	...
	for kind in FACILITIES + SCENERY + PROPS:
		var model: Node3D = AssetFactory.build(kind)
		assert(model != null, "AssetFactory returned null for " + kind)
		assert(model.get_child_count() > 0, "Asset has no visible parts: " + kind)
